...
(function() {
    ...
    function showChart(target, points) {
        ...
        var isIndividual = points.length > 0 && points[0].rtt_ms !== undefined;
        
        var validPoints, lostPoints = [];
        if (isIndividual) {
            validPoints = points.filter(function(p) { return p.rtt_ms !== null && !p.lost; });
            lostPoints = points.filter(function(p) { return p.lost; });
        } else {
            validPoints = points.filter(function(p) { return p.avg_ms !== null; });
        }
        ...
        var html = '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">' +
            ...
            '<button type="button" class="btn btn-default" data-range="10080">1w</button>' +
            '<button type="button" class="btn btn-default" data-range="43200">1M</button>' +
            '<button type="button" class="btn btn-default" data-range="129600">3M</button>' +
            '<button type="button" class="btn btn-default" data-range="259200">6M</button>' +
            '<button type="button" class="btn btn-default" data-range="525600">1Y</button>' +
            '<button type="button" class="btn btn-default" data-range="1576800">3Y</button>' +
            '</div></div>';
        ...
        if (validPoints.length > 0) {
            var currentVal, minLatency, maxLatency, lossPercent;
            if (isIndividual) {
                var rttValues = validPoints.map(function(p) { return p.rtt_ms; });
                currentVal = rttValues[rttValues.length - 1] || 0;
                minLatency = Math.min.apply(null, rttValues);
                maxLatency = Math.max.apply(null, rttValues);
                lossPercent = (lostPoints.length / points.length * 100);
            } else {
                ...
                currentVal = avgValues[avgValues.length - 1] || 0;
                minLatency = minValues.length ? Math.min.apply(null, minValues) : 0;
                maxLatency = maxValues.length ? Math.max.apply(null, maxValues) : 0;
                lossPercent = (lossCount / validPoints.length * 100);
            }
            
            html += '<div class="row" style="margin-top: 10px;">' +
                '<div class="col-xs-3 text-center"><div style="font-size: 18px; font-weight: bold; color: #337ab7;">' + currentVal.toFixed(2) + '</div><div style="font-size: 11px; color: #999;">Current (ms)</div></div>' +
                ...
                '<div class="col-xs-3 text-center"><div style="font-size: 18px; font-weight: bold; color: ' + (lossPercent > 0 ? '#d9534f' : '#5cb85c') + ';">' + lossPercent.toFixed(1) + '%</div><div style="font-size: 11px; color: #999;">Loss</div></div>' +
                '</div>';
        }
        ...
        // Draw D3 chart
        drawD3Chart(points, isIndividual);
    }
    
    function drawD3Chart(points, isIndividual) {
        ...
        if (isIndividual) {
            ...
            points.forEach(function(p) {
                ...
                if (!groupedByTime[key]) {
                    groupedByTime[key] = { ts: new Date(p.ts), rtts: [], lostCount: 0 };
                }
                if (p.lost) {
                    groupedByTime[key].lostCount++;
                } else if (p.rtt_ms !== null && p.rtt_ms !== undefined) {
                    groupedByTime[key].rtts.push(p.rtt_ms);
                }
            });
            
            // Convert to sorted array with sorted RTTs per timestamp
            var timePoints = Object.values(groupedByTime)
                .sort(function(a, b) { return a.ts.getTime() - b.ts.getTime(); })
                .map(function(g) {
                    return {
                        ts: g.ts,
                        rtts: g.rtts.sort(function(a, b) { return a - b; }),
            ...
            var allRtts = timePoints.flatMap(function(p) { return p.rtts; });
            var yMax = d3.max(allRtts) || 100;
            var yScale = d3.scaleLinear().domain([0, yMax * 1.1]).range([innerHeight, 0]);
            
            // Colors from darkest (lowest RTT) to lightest (highest RTT)
            var smokeColors = ['#1e40af', '#2563eb', '#3b82f6', '#60a5fa', '#93c5fd'];
            
            // Draw filled areas between each RTT level
            for (var level = 0; level < 5; level++) {
                var areaData = timePoints.filter(function(p) { return p.rtts.length > level; });
                if (areaData.length < 2) continue;
                
                var areaGen = d3.area()
                    .x(function(d) { return xScale(d.ts); })
                    .y0(function(d) { return level === 0 ? innerHeight : yScale(d.rtts[level - 1] || 0); })
                    .y1(function(d) { return yScale(d.rtts[level] || 0); })
            ...
            for (var level = 0; level < 5; level++) {
                var lineData = timePoints.filter(function(p) { return p.rtts.length > level; });
                if (lineData.length < 2) continue;
                
                var lineGen = d3.line()
                    .x(function(d) { return xScale(d.ts); })
                    .y(function(d) { return yScale(d.rtts[level] || 0); })
                    .curve(d3.curveMonotoneX);
            ...
            var lostTimePoints = timePoints.filter(function(p) { return p.lostCount > 0; });
            g.selectAll('.lost-marker')
                .data(lostTimePoints)
                .enter()
                .append('rect')
                .attr('x', function(d) { return xScale(d.ts) - 2; })
            ...
            g.append('rect')
                ...
                .on('mousemove', function(event) {
                    ...
                    var i = bisect(timePoints, x0, 1);
                    var d0 = timePoints[i - 1];
                    var d1 = timePoints[i];
                    if (!d0) return;
                    var d = d1 && (x0.getTime() - d0.ts.getTime() > d1.ts.getTime() - x0.getTime()) ? d1 : d0;
                    
                    var xPos = xScale(d.ts);
                    hoverLine.attr('x1', xPos).attr('x2', xPos).style('opacity', 1);
                    
                    var tooltipHtml = '<div style="margin-bottom: 4px; color: #999;">' + d.ts.toLocaleTimeString() + '</div>';
                    if (d.rtts.length > 0) {
                        tooltipHtml += '<div><span style="color: #60a5fa;">RTT:</span> ' + d.rtts.map(function(r) { return r.toFixed(2); }).join(', ') + ' ms</div>';
                    }
                    if (d.lostCount > 0) {
                        tooltipHtml += '<div style="margin-top: 4px; padding-top: 4px; border-top: 1px solid #444;"><span style="color: #f87171;">Lost:</span> ' + d.lostCount + ' packets</div>';
                    }
            ...
            var yMax = d3.max(validPoints, function(d) { return d.max_ms || d.avg_ms || 0; }) * 1.1;
            ...
            // Loss markers
            var lossPoints = validPoints.filter(function(p) { return (p.loss_pct || 0) > 0; });
            ...
            g.append('rect')
                ...
                .on('mousemove', function(event) {
                    var mouseX = d3.pointer(event)[0];
                    var x0 = xScale.invert(mouseX);
                    var i = bisect(validPoints, x0, 1);
                    var d0 = validPoints[i - 1];
                    var d1 = validPoints[i];
                    if (!d0) return;
